...
O portfólio também disponibiliza uma página própria em **`/privacidade`**, acessível pelo link discreto **Privacidade** no rodapé.

A página descreve o funcionamento atual do site: não há cadastro de usuários, login, banco de dados próprio de visitantes ou formulário interno de coleta de dados; também não foram identificados cookies publicitários, Google Analytics, Meta Pixel ou ferramentas similares no código atual. Ela informa ainda sobre os links para serviços externos e sobre o processamento técnico que pode ser realizado pela infraestrutura de hospedagem da Vercel, como logs necessários ao funcionamento e à segurança.

A rota foi implementada sem adicionar uma biblioteca de roteamento. `src/main.tsx` identifica de forma simples o caminho `/privacidade`, enquanto `vercel.json` possui o rewrite necessário para permitir acesso direto e refresh nessa URL.
...
O portfólio utiliza headers HTTP de segurança configurados na Vercel por meio de `vercel.json`.

As proteções atualmente configuradas incluem:

- **`X-Content-Type-Options: nosniff`**;
- **`Referrer-Policy: strict-origin-when-cross-origin`**;
- **`X-Frame-Options: DENY`**;
- **`Permissions-Policy`** com câmera, microfone, geolocalização, pagamentos e USB desabilitados;
- **`Content-Security-Policy`** específica para os recursos utilizados pela aplicação.

A CSP atual é restritiva e permite apenas os recursos necessários ao funcionamento do portfólio:
...
A política não utiliza `*`, `unsafe-eval` ou `unsafe-inline`. Os links externos para WhatsApp, Instagram, GitHub e projetos publicados continuam funcionando como navegação externa, sem conceder a esses domínios permissão para executar scripts dentro do portfólio.

Nenhum banner de cookies foi adicionado, pois não foram identificados trackers publicitários ou ferramentas de analytics no código atual.
...
The page explains that the current site has no user registration, login, first-party visitor database or internal data-collection form; no advertising cookies, Google Analytics, Meta Pixel or similar tools were identified in the current code. It also explains that external services have their own privacy policies and that the Vercel hosting infrastructure may process technical information such as logs required for operation and security.

The route is implemented without adding a routing library. `src/main.tsx` performs a small pathname check, while `vercel.json` provides the rewrite required for direct access and refresh on `/privacidade`.

---

### Security and privacy

HTTP security headers are configured through `vercel.json`:

- **`X-Content-Type-Options: nosniff`**;
- **`Referrer-Policy: strict-origin-when-cross-origin`**;
- **`X-Frame-Options: DENY`**;
- **`Permissions-Policy`** restricting unused browser capabilities;
- **`Content-Security-Policy`** limiting the origins allowed for application resources.
...
The policy does not use `*`, `unsafe-eval` or `unsafe-inline`. External links remain available as normal navigation without granting those external domains permission to execute scripts inside the portfolio.
...
├─ main.tsx                       # Application entry point and lightweight route selection
├─ privacy.css                    # Privacy page styles
└─ *.css                          # Layout, themes, previews and responsiveness
vercel.json                       # Security headers, CSP and /privacidade rewrite
```
